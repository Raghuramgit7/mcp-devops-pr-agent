...
async def review_pr(installation_id: int, repo_name: str, pr_number: int, conversation_id: str):
    """
    ...
    try:
        logger.info(f"[{conversation_id}] Starting review process for {repo_name}#{pr_number}")
        # 1. Fetch Diff via MCP
        owner, repo = repo_name.split("/")
        logger.info(f"[{conversation_id}] Calling MCP tool 'get_pr_diff' for {owner}/{repo}")
        ...
        logger.info(f"[{conversation_id}] Diff fetched successfully ({len(diff_text)} chars). Querying Gemini...")
        ...
        for attempt in range(3):
            try:
                ...
                if "429" in str(e) and attempt < 2:
                    ...
                    logger.warning(f"[{conversation_id}] Rate limit hit in reviewer. Retrying in {wait_time}s...")
                    await asyncio.sleep(wait_time)
                else:
                    raise e

        if not response:
            logger.error(f"[{conversation_id}] Failed to get review from Gemini after retries")
            return
