...
async def run_quality_review(
    ...
    try:
        logger.info(
            f"[{conversation_id}] Starting quality review for {repo_name}#{pr_number}"
        )
        ...
        if hasattr(quality_result, 'content'):
            quality_text = quality_result.content[0].text
        else:
        ...
        Please summarize the most critical security vulnerabilities and code quality issues.
        - Ignore minor style warnings if they are overwhelming.
        - Focus on hardcoded secrets, insecure function calls, or critical logic errors.
        ...
        for attempt in range(3):
            try:
                ...
                if "429" in str(e) and attempt < 2:
                    ...
                    logger.warning(
                        f"[{conversation_id}] Rate limit hit in quality_checker."
        ...
        if "Code quality and security look solid!" not in summary:
            ...
            logger.info(
                f"[{conversation_id}] Posted quality & security report for PR #{pr_number}"
            )
        else:
