...
async def check_pr_docs(
    ...
    try:
        logger.info(
            f"[{conversation_id}] Checking documentation for {repo_name}#{pr_number}"
        )
        ...
        If you find missing docstrings that can be easily added, provide them in a JSON block.
        ...
        for attempt in range(3):
            try:
                ...
                if "429" in str(e) and attempt < 2:
                    ...
                    logger.warning(
                        f"[{conversation_id}] Rate limit hit in doc_checker."
        ...
        if not response:
            logger.error(
                f"[{conversation_id}] Failed to get response from Gemini after retries"
            )
            return
        ...
        if json_match:
            try:
                ...
                commit_msg = fix_data.get(
                    "commit_message", "docs: AI auto-generated docstrings"
                )

                if file_path and new_content:
                    logger.info(
                        f"[{conversation_id}] Auto-docs fix for"
                        f" {file_path} on {branch_name}"
                    )
                ...
                logger.error(
                    f"[{conversation_id}] Failed to apply auto-docs JSON: {json_err}"
                )
        ...
        if "Documentation looks great!" not in suggestions or fix_applied:
            ...
            if fix_applied:
                review_body += (
                    f"\n\n---\n**📝 AI Documentation Fix Applied**: "
                    f"Added docstrings to `{file_path}` on branch `{branch_name}`."
                )

            msg = f"## 📚 Documentation Review\n\n{review_body}"
            post_comment(installation_id, repo_name, pr_number, msg)
            logger.info(
                f"[{conversation_id}] Posted documentation report for PR #{pr_number}"
            )
        else:
